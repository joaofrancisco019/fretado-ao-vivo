...
def get_cached_lines(hash_code):
    ...
    cache_file = f"cache_lines_{hash_code}.json"
...
class CustomHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/api/find-closest-lines'):
            self.handle_find_closest_lines()
            return

        # 3. Live Traffic ETA
        if self.path.startswith('/api/traffic-eta'):
    ...
    def handle_geocode(self):
        ...
        # 1. Check if user pasted direct coordinates or a Google Maps URL
        # E.g.: -22.8433, -47.0541
        ...
        if coord_match:
            ...
            self.send_json_response([{
                ...
                'display': f"Coordenadas do Google Maps: {lat}, {lon}",
                'lat': lat,
                'lon': lon
            }])
            return

        # Check if user pasted a Google Maps short link (maps.app.goo.gl or goo.gl/maps)
                ...
                with urllib.request.urlopen(req, timeout=6) as r:
                    ...
                    if m:
                        ...
                        self.send_json_response([{
                            ...
                            'display': f"Link do Google Maps resolvido ({lat:.4f}, {lon:.4f})",
                            'lat': lat,
                            'lon': lon
                        }])
                        return
        ...
        try:
            ...
            print("Photon error, falling back to Nominatim:", e)
...
if __name__ == "__main__":
    ...
    with socketserver.TCPServer(("", PORT), CustomHandler) as httpd:
        ...
        print("=" * 65)
        print("  [1;32mFRETADO AO VIVO - PWA SERVER[0m")
        print("=" * 65)
        print(f"  * Acesso no Computador: [1;34mhttp://localhost:{PORT}[0m")
        print(f"  * Acesso no Celular (mesmo Wi-Fi): [1;34mhttp://{local_ip}:{PORT}[0m")
        print("=" * 65)
